...
export function HistorialTimeline({ compraId }: HistorialTimelineProps) {
  ...
  if (historial.length === 0) {
    return (
      <div className="text-center p-4 text-muted-foreground text-sm">No hay historial registrado para esta compra.</div>
    );
  }
          ...
          <div key={log.id} className="relative pl-6 border-l-2 border-muted pb-1 last:pb-0">
            ...
            <div className="flex flex-col gap-1">
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-sm font-medium">{log.expand?.usuario?.name || "Usuario desconocido"}</span>
                    ...
                    {Object.entries(log.cambios).map(([key, diff]) => (
                      <li key={key} className="flex flex-col sm:flex-row sm:gap-2">
                        ...
                        <span>
                          ...
                          <span className="text-green-600 font-medium">{String(diff.nuevo ?? "Vacío")}</span>
                        </span>
                      </li>
                    ))}
                  </ul>
