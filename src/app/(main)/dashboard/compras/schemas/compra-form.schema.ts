...
export function createCompraFormSchema(roles: UserRole[], context: { isCreating: boolean } = { isCreating: false }) {
  ...
  const isRequired = (field: string) => {
    ...
    if (context.isCreating) {
      return ["numero_ordinario", "unidad_requirente", "descripcion", "estado", "comprador", "fecha_inicio"].includes(
        field,
      );
    }
  ...
  return z.object({
    ...
    adjunta_ordinario: z
      .instanceof(File)
      .optional()
      .refine((file) => !file || file.size <= MAX_FILE_SIZE, "El tamaño máximo de archivo es 10MB.")
      .refine((file) => !file || ACCEPTED_FILE_TYPES.includes(file.type), "Solo se aceptan archivos PDF e imágenes."),
